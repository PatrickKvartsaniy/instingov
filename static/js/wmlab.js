...
$(document).ready(function () {
    ...
    function ajaxSubmit() {
        $(document.body).on('submit', 'form', function (event) {
            ...
            $(formDetails).find('[type="checkbox"]').each(function () {
                if ($(formDetails).find('[type="hidden"][name="' + $(this).attr('name') + '"]').length) {
                    $(formDetails).find('[type="hidden"][name="' + $(this).attr('name') + '"]').remove();
                }
                if (!$(this).is(':checked')) {
                    var t = $(this).attr('data-off-value');
                    if (typeof t === "undefined") {
                        var field = '<input type="hidden" name="' + $(this).attr('name') + '" value="0" />';
                    } else {
                        var field = '<input type="hidden" name="' + $(this).attr('name') + '" value="' + $(this).attr('data-off-value') + '" />';
                    }
                }
    ...
    function ajaxProcess(formDetails) {
        ...
        toastr.info('Processing..', '', {"positionClass": toastr_position, "fadeOut": 40000, "timeOut": 50000, "extendedTimeOut": 50000});
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status == "error") {
                    toastr.error(response.message, '', {"positionClass": toastr_position, "fadeOut": 40000, "timeOut": 50000, "extendedTimeOut": 50000});
                }
                else if (response.status == "need_2f_confirm") {
                    lpbf = formDetails;
                    if (response.html) {
                        $("#need_confirm_modal_body").html(response.html);
                        $("#need_confirm_modal").modal('show');
                    } else {
                        toastr.error(response.message, '', {"positionClass": toastr_position, "fadeOut": 40000, "timeOut": 50000, "extendedTimeOut": 50000});
                    }
                }
                else {
                    if (formDetails.hasClass('two-factor-auth-form') && (typeof(lpbf) !== "undefined") && (lpbf !== null)) {
                        setTimeout(function () {
                            $(lpbf).find('[type="submit"]').first().click();
                            lpbf = null;
                        }, 500);
                    }

                    if (response.message)
                        toastr.success(response.message, '', {"positionClass": toastr_position, "fadeOut": 40000, "timeOut": 50000, "extendedTimeOut": 50000});
                    $('#myModal').modal('hide');
            ...
            error: function (response) {
                ...
                if (response.status === 422) {
                    ...
                    toastr.error(errorsHtml, '', {"positionClass": toastr_position, "fadeOut": 40000, "timeOut": 50000, "extendedTimeOut": 50000});
                } else
                    toastr.error(something_error_message, '', {"positionClass": toastr_position, "fadeOut": 40000, "timeOut": 50000, "extendedTimeOut": 50000});
            },
            complete: function (responseData) {
                ...
                if (typeof response !== 'undefined' && response && typeof response.url !== 'undefined') {
                    if (response.timeout) {
                        setTimeout(function () {
                            window.location.href = response.url;
                        }, response.timeout * 1000);
                    } else {
                        if(response.url.toLowerCase().trim() == 'reload' || response.url.toLowerCase().trim() == 'refresh' || response.url.trim() == '') {
                            setTimeout(function () {
                                location.reload();
                            }, 1000);
                        } else if(response.url.toLowerCase().trim() == 'reloadNow' || response.url.toLowerCase().trim() == 'refreshNow') {
                            location.reload();
                        } else {
                            setTimeout(function () {
                                window.location.href = response.url;
    ...
    function clearForm(ele) {
        if ($(ele).find('[data-dismiss="modal"]:first').length) {
            $(ele).find('[data-dismiss="modal"]:first').click();
        }
